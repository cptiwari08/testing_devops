...
class PMOTool(EYIPBaseTool):
    """
    A class used to handle the PMO (Project Management Office) tool.
    This class provides methods to initialize the PMO tool and generate a QueryEngineTool.
    It inherits from the EYIPBaseTool base class.
    """

    def __init__(self, logger, llm_models) -> None:
        super().__init__(logger, llm_models)
        self._logger.info("Initializing PMOTool")
        self._engine = MSSQLServerManager(config, self._logger).get_engine()
        self._tables_names = ["WorkPlan"]
        self._prompt_manager = create_prompt_manager(self._logger)

    def generate(self) -> QueryEngineTool:
        """
        Generates a QueryEngineTool for the PMO tool.
        Returns:
            A QueryEngineTool for the PMO tool.
        """
        self._logger.info("PMOTool Getting SQL Connection")
        ...
        self._logger.info("PMOTool generated")
        return sql_tool

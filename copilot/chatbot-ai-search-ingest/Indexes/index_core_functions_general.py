...
class AISearchCollection:
    ...
    def __init__(self):
        self.__instances = [AISearch(ia.strip(),os.getenv("PRJDATA_GENERAL_INDEX")) for ia in os.getenv("AIS_INSTANCE").split(",")]
    ...
    def process_all_instances(self, index_name, doc, fields):
        """
        ...
        for ai_search in self.__instances:
            try:
                ai_search.process_all_in_one(index_name,doc,fields)
...
class AISearch:
    ...
    def __init__(self, instance_name, index_name):
        # AI Search variables
        ...
        self.__search_client = SearchClient(endpoint=self.__search_service_endpoint,index_name=index_name,credential=self.__credential)
    ...
    def load_index(self, index_name, doc):
        """
    ...
    def process_all_in_one(self, index_name, doc, fields):
        ...
        for i in range(0,len(doc),700):
            ...
            self.load_index(index_name=index_name,doc=json_load_batch)
            print(len(doc[i:i+700])," documents loaded")
...
class FieldHandler:
    ...
    def __init__(self):
        ...
        self.add_str_field({"name": "indexType", "retrievable": True, "filterable": True})
    ...
    #    return llm.embeddings.create(input=text, model=azure_openai_embedding_model, dimensions=1536).data[0].embedding
